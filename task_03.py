...
def get_time_summary(ar: dict):
    lesson = ar['lesson']
    pupil = sorted(ar['pupil'])
    tutor = ar['tutor']
    total = []
    intersection = 0
    for l_start, l_end in itertools.combinations(lesson, 2):
        for t_start, t_end in itertools.combinations(tutor, 2):
            for p_start, p_end in itertools.combinations(pupil, 2):
                if max(p_start, t_start, l_start) <= min(p_end, t_end, l_end):
                    total.append(max(p_start, t_start, l_start))
                elif max(p_start, t_start, l_start) >= min(p_end, t_end, l_end):
                    total.append(min(p_end, t_end, l_end))
                total.append(min(lesson[-1], ar['pupil'][-1], ar['tutor'][-1]))

    res = sorted(set(total))
    for start, end in zip(res[::2], res[1::2]):
        intersection += int(end) - int(start)
    return intersection
...
if __name__ == '__main__':
    for i, test in enumerate(tests):
        test_answer = get_time_summary(test['data'])
